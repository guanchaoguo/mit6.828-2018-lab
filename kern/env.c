...
env_setup_vm(struct Env *e) {
    ...
    e->env_pgdir = upgdir;  //刚分配的物理页作为页目录使用
...
region_alloc(struct Env *e, void *va, size_t len) {
    ...
    while (begin < end) {
        ...
        int ret = page_insert(e->env_pgdir, pg, begin, PTE_W | PTE_U);   //修改e->env_pgdir，建立线性地址begin到物理页pg的映射关系
        if (ret == -E_NO_MEM) {
...
load_icode(struct Env *e, uint8_t *binary) {
    ...
    struct Proghdr *phEnd = (struct Proghdr *) (ph + elfHeader->e_phnum);
